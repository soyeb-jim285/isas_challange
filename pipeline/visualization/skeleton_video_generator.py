...
class SkeletonVideoGenerator:
    def __init__(self, keypoint_file: str, frame_mapping_file: str, output_path: str = "results/videos/"):
        """
        Initialize the skeleton video generator.
    ...
    def draw_skeleton(self, frame: np.ndarray, keypoints: List[Tuple[int, int]], 
        ...
        for i, (x, y) in enumerate(scaled_keypoints):
            if x > 0 and y > 0:  # Only draw if coordinates are valid
                cv2.circle(frame_with_skeleton, (x, y), 4, (255, 255, 255), -1)
                cv2.circle(frame_with_skeleton, (x, y), 6, (0, 0, 0), 1)
        ...
        for connection in self.skeleton_connections:
            ...
            if (start_idx < len(scaled_keypoints) and end_idx < len(scaled_keypoints) and
                ...
                cv2.line(frame_with_skeleton, start_point, end_point, (255, 255, 255), 3)
        
        # Draw action label with background
        ...
        # Draw text
        cv2.putText(frame_with_skeleton, label, (20, 20 + text_height), 
                   font, font_scale, color, thickness)
        
        # Draw frame number
        frame_label = f"Frame: {frame_num}"
        cv2.putText(frame_with_skeleton, frame_label, (20, frame_with_skeleton.shape[0] - 20), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        return frame_with_skeleton
    
    def generate_video(self, fps: int = 30, resolution: Tuple[int, int] = (854, 480), max_frames: int = None):
        """Generate the skeleton video with action labels."""
        print("Loading data...")
        ...
        if max_frames:
            keypoints_df = keypoints_df.head(max_frames)
            frame_mapping_df = frame_mapping_df.head(max_frames)
        
        # Create video writer
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        output_filename = os.path.join(self.output_path, "skeleton_action_video_fixed.mp4")
        out = cv2.VideoWriter(output_filename, fourcc, fps, resolution)
        
        print(f"Generating video with {len(keypoints_df)} frames...")
        
        # Process each frame
        for frame_idx in range(len(keypoints_df)):
            if frame_idx % 1000 == 0:
                print(f"Processing frame {frame_idx}/{len(keypoints_df)}")
...
def main():
    """Main function to generate the skeleton video."""
    # File paths
    ...
    # Create generator
    generator = SkeletonVideoGenerator(keypoint_file, frame_mapping_file)
    
    # Generate video (limit to first 5000 frames for testing)
    output_file = generator.generate_video(fps=30, resolution=(854, 480), max_frames=5000)
