#!/usr/bin/env python3
"""
Generate full skeleton video with action labels for all frames.
"""
...
class SkeletonVideoGenerator:
    ...
    def generate_video(self, fps: int = 30, resolution: Tuple[int, int] = (854, 480)):
        ...
        output_filename = os.path.join(self.output_path, "full_skeleton_action_video_fixed.mp4")
        out = cv2.VideoWriter(output_filename, fourcc, fps, resolution)
        
        print(f"Generating full video with {len(keypoints_df)} frames...")
        ...
        out.release()
        print(f"Full video saved to: {output_filename}")
        
        return output_filename

def main():
    """Main function to generate the full skeleton video."""
    # File paths
    ...
    print(f"Full video generation complete: {output_file}")
